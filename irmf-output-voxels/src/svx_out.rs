    ...
    P: FnMut(usize, usize),
{
    ...
    .map_err(|_| {
        IrmfError::IoError(std::io::Error::new(
            std::io::ErrorKind::Other,
            "Invalid current local time for SVX",
        ))
    })?;
    ...
    // 1. Write Manifest
    zip.start_file("manifest.xml", options).map_err(|e| {
        IrmfError::IoError(std::io::Error::new(
            std::io::ErrorKind::Other,
            e.to_string(),
        ))
...
<grid version="1.0" gridSizeX="{}" gridSizeY="{}" gridSizeZ="{}"
   voxelSize="{:.6}" subvoxelBits="8" slicesOrientation="Z" >
    ...
    slicer.render_z_slices(material_num, |z_idx, _z, _radius, img| {
        ...
        zip.start_file(slice_name, options).map_err(|e| {
            IrmfError::IoError(std::io::Error::new(
                std::io::ErrorKind::Other,
                e.to_string(),
            ))
